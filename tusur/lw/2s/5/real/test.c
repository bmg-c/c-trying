...
#define MAX_ELEMENETS_AMOUNT 1000000
#define STEP 10000
...
int merge(int *arr, int first, int mid, int last) {
    long long int comp_swap_amount = 0;
...
void run_tests(char *file_name) {
    ...
    for (int elements_amount = STEP; elements_amount <= MAX_ELEMENETS_AMOUNT;
         elements_amount += STEP) {
        long long int average_comparison_swap = 0;
        ...
        fprintf(csv_file, "%d; %lld\n", elements_amount,
                average_comparison_swap);
        printf("%d; %lld\n", elements_amount, average_comparison_swap);
    }
